...
# FINAL FIX: Explicitly define both routes (/strings and /strings/)
...
# FINAL FIX: Explicitly define both routes for specific strings
...
# FINAL FIX: Explicitly define both routes for natural language filter
...
def filter_natural_language():
    ...
    if 'longer than' in query:
        try:
            parts = query.split('longer than')[1].strip().split()
            if parts: length = int(parts[0])
            else: return jsonify({"error": "Unable to parse length from query"}), 400
    ...
    if 'word_count' in parsed_filters: filtered_data = [s for s in filtered_data if s['properties']['word_count'] == parsed_filters['word_count']]
